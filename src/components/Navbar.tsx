...
export const Navbar: React.FC<NavbarProps> = ({ onProfileClick }) => {
  ...
  const handleNotifications = () => {
    toast.info('No new notifications', {
      description: 'Your credit vibe is currently stable.',
      position: 'top-center',
    });
  };
      ...
      <div className="flex items-center gap-3">
        {user && (
          <button 
            ...
            className="w-10 h-10 rounded-full overflow-hidden border-2 border-primary-container shrink-0 hover:scale-105 active:scale-95 transition-transform"
          >
            {user.photoURL ? (
              <img 
                src={user.photoURL} 
